...
export class OpenCodeSessionTracer {
  ...
  constructor(config: Config, inputConfig?: Partial<RunTreeConfig>) {
    this.inputConfig = inputConfig ?? {};
    this.inputConfig.project_name ??= config.project;
    this.inputConfig.replicas ??= config.replicas;
    this.inputConfig.tracingEnabled ??= config.enabled;
  ...
  private getMessage(sessionID: string, messageID: string): AggregateMessage {
    ...
    if (session.pendingSystem != null && session.messages[messageID]?.info?.role === "assistant") {
    ...
    return {
      ...
      usage_metadata: stepFinish
        ? {
            ...
            total_tokens:
              stepFinish.tokens.input + stepFinish.tokens.output + stepFinish.tokens.reasoning,
            input_token_details: {
    ...
    for (const run of agentRuns) {
      ...
      for (const toolPart of parts) {
        ...
        const tool = child.createChild({
          ...
          outputs: {
            output: state.output,
            attachments: state.attachments?.map(convertToStandardContentBlock) ?? undefined,
          },
